...
\begin{itemize}
	...
	\item { Các trường dữ liệu:
	      \begin{itemize}
		      \item province: Tên thành phố \emph{(không dấu)}
		      \item date: ngày
		      \item max: nhiệt độ cao nhất trong ngày \emph{(celcius)}
		      \item min: nhiệt độ thấp nhất trong ngày \emph{(celcius)}
		      \item wind: tốc độ gió \emph{(km/h)}
		      \item wind\_d: hướng gió \href{https://www7.ncdc.noaa.gov/climvis/help_wind.html}{direction}
		      \item rain: lượng mưa \emph{(mm)}
		      \item humidi: độ ẩm \emph{(\%)}
		      \item cloud: mây \emph{(\%)}
		      \item pressure: áp suất(\href{https://en.wikipedia.org/wiki/Bar_(unit)}{Bar})
	      \end{itemize}
	      }
\end{itemize}
